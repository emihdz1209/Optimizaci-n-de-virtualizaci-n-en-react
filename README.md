...
> **Importante:** Aunque un componente hijo no use el estado del padre, si el padre se renderiza, por defecto el hijo también lo hace. Esto puede provocar renderizados innecesarios.

---

## React.memo

`React.memo` es una función de orden superior (**Higher Order Component**) que memoriza un componente funcional. Evita que el componente se vuelva a renderizar si sus props no han cambiado.

Cuando un componente está envuelto en `React.memo`:
- React compara las props anteriores con las nuevas.
- Si son iguales (**shallow comparison**), el componente **no** se renderiza nuevamente.
...
|---|---|
| Única | No debe repetirse entre elementos. |
| Estable | No debe cambiar entre renders. |
| No usar índice | Usar el índice del array como `key` es un error común. |
